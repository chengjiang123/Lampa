...
from ..model_utils.hash_utils import lsh_mapping, lsh_mapping1, batched_index_select, invert_permutation, E2LSH


def sort_to_buckets(x, perm, bucketsz):
    #print(f'before buckets {x.shape}')
    return rearrange(
...
def process_hidden_segments(sorted_hidden,sorted_processed_hidden):
    """
    Process the sorted hidden states by computing pairwise distances and aggregating
    them for the localized segments (similar to qkv_res but adapted for hidden states).
    """
    # Compute squared norm for each segment of hidden states (like q^2 and k^2)
    hidden_sq_05 = -0.5 * (sorted_hidden ** 2).sum(dim=-1, keepdim=True)

    # Compute pairwise distances (or similarities) between hidden state segments
    hidden_dists = torch.einsum("...id,...jd->...ij", sorted_hidden, sorted_hidden)
    
    # Add squared norms and apply the transformation (like in qkv_res)
    hidden_dists = (hidden_dists + hidden_sq_05 + hidden_sq_05.transpose(-1, -2)).clamp(max=0.0).exp()

    # Compute the denominator (normalization factor) for scaling the output
    denom = hidden_dists.sum(dim=-1, keepdim=True) + (1e-20)

    # Perform aggregation based on the distances
    processed_output = torch.einsum("...ij,...jd->...id", hidden_dists, sorted_processed_hidden)

    return denom, processed_output
...
def prep_hidden(hidden_states, w, coords):
    """
    Preprocess hidden states by incorporating positional embeddings (coords) and
    transformations applied by w.
    """
    # Similar to how 'query' was processed, we apply the same to the hidden states
    hw = w.sum(dim=1).clamp(max=50).exp().sum(dim=-1)
    new_hw_expand_dim = torch.cat([hw[:, :1], hw], dim=-1)

    # Apply the positional embeddings to the hidden states
    sqrt_w_r = torch.sqrt(2 * new_hw_expand_dim)[None] * coords[:, None]

    # Concatenate the hidden state with the positional embedding information
    hidden_hat = torch.cat([hidden_states, sqrt_w_r], dim=-1)
