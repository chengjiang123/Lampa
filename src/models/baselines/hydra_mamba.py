...
from ..attention import (
    HEPTAttention,CausalSelfAttention
)
...
class Encoder(nn.Module):  ## Embedding module
    ### change dimension
    def __init__(self, in_dim, hidden_dim, encoder_channel):
        super().__init__()
        ...
        self.first_conv = nn.Sequential(
            nn.Conv1d(self.in_dim, self.hidden_dim//2, 1),
            #nn.BatchNorm1d(self.hidden_dim//2),
            nn.ReLU(inplace=True),
            nn.Conv1d(self.hidden_dim//2, self.hidden_dim, 1)
        )
        self.second_conv = nn.Sequential(
            nn.Conv1d(self.hidden_dim * 2, self.hidden_dim * 2, 1),
            #nn.BatchNorm1d(self.hidden_dim * 2),
            nn.ReLU(inplace=True),
            nn.Conv1d(self.hidden_dim * 2, self.encoder_channel, 1)
        )

    def forward(self, point_groups):
        '''
            point_groups : B G N D
            -----------------
            feature_global : B G C
        '''
        bs, g, n, _ = point_groups.shape
        point_groups = point_groups.reshape(bs * g, n, self.in_dim)
        # encoder
        feature = self.first_conv(point_groups.transpose(2, 1))  # BG self.hidden_dim n
        feature_global = torch.max(feature, dim=2, keepdim=True)[0]  # BG self.hidden_dim 1
        feature = torch.cat([feature_global.expand(-1, -1, n), feature], dim=1)  # BG self.hidden_dim * 2 n
        feature = self.second_conv(feature)  # BG self.hidden_dim * 2 *2 n
        feature_global = torch.max(feature, dim=2, keepdim=False)[0]  # B G self.hidden_dim * 2 * 2
...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, features, xyz):
        ...
        # fps the centers out
        center = fps(xyz, num_points)  # B G 3
        _, idx = self.knn(xyz, center)  # B G M
        #print(f'idx group {idx.shape}')
        assert idx.size(1) == num_points
        assert idx.size(2) == self.group_size
        idx_base = torch.arange(0, batch_size, device=xyz.device).view(-1, 1, 1) * num_points
        #print(f'idx_base group {idx_base.shape}')
        idx = idx + idx_base
        idx = idx.view(-1)
        neighborhood_xyz = xyz.view(batch_size * num_points, -1)[idx, :]
        neighborhood_xyz = neighborhood_xyz.view(batch_size, num_points, self.group_size, 6).contiguous()
        # normalize
        neighborhood_xyz = neighborhood_xyz - center.unsqueeze(2)
        #print(f'initial group {features.shape}')
        #print(f'len idx {len(idx)}')
        neighborhood = features.view(batch_size * num_points, -1)[idx, :]
...
class HydraMamba2(nn.Module):
    def __init__(self, in_dim, task,  args: Mamba2Config, **kwargs):
        ...
        self.encoder = Encoder(in_dim=6,hidden_dim=self.hidden_dim,encoder_channel=self.encoder_dims)
        
        if self.task == "pileup":
            self.pids_enc = nn.Embedding(7, 10)
            in_dim = in_dim - 1 + 10
            self.out_proj = nn.Linear(int(self.hidden_dim), 1)
            self.norm2 = nn.LayerNorm(int(self.hidden_dim))
            self.ff = nn.Sequential(
                nn.Linear(int(self.hidden_dim), int(self.hidden_dim)),
                nn.ReLU(),
                nn.Linear(int(self.hidden_dim), int(self.hidden_dim)),
            )
    ...
    def forward(self, data):
        ...
        ## Not only locations but all variables for x
        ...
        ## we could consider this if we have a better definition of classes. but now just sig
        if self.task == "pileup":
...
class FlashAtt(nn.Module):
    def __init__(self, **kwargs):
        ...
        # +2 for data.pos
        self.attn = CausalSelfAttention(self.h_dim, **kwargs)
    ...
    def forward(self, x, kwargs):
        x_normed = self.norm1(x)
        aggr_out = self.attn(x_normed, **kwargs)
