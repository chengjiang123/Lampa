...
#config_path = Path('rwkv.yaml')
#with open(config_path, 'r') as file:
#    config0 = yaml.safe_load(file)

# Extract head_size_a from config
#head_size_a = config0['model_kwargs']['head_size_a']
...
MyFunction = __nop
base_path = "/eos/user/c/chjiang/Lampa/src/models/rwkv/"
## prepare input
...
wkv5_cuda = load(name="wkv5", sources=[f"{base_path}/cuda/wkv5_op.cpp", f"{base_path}/cuda/wkv5_cuda.cu"],
...
class RWKV_Tmix_x052(nn.Module):
    ...
    def jit_func(self, x):
        ...
        #print(f'####')
        #print(f'xw {xk.shape}')
        #print(f'time_decay {self.time_decay.shape}')
        #print(f'time_decay_w1 {self.time_decay_w1.shape}')
        #print(f'time_decay_w2 {self.time_decay_w2.shape}')
...
class RWKV(nn.Module):
    def __init__(self, in_dim, task, **kwargs):
        ...
        if self.task == "pileup":
            ...
            in_dim = in_dim - 1 + 10
            self.out_proj = nn.Linear(int(self.n_embd), 1)

        self.n_layer = kwargs["n_layers"]
        self.n_mixedattn_layers = kwargs["n_mixedattn_layers"]
        ...
        self.head = nn.Linear(self.n_embd, int(self.n_embd), bias=False)

        self.init_params() 
        
        self.dropout = nn.Dropout(0.1)
        self.mlp_out = MLP(
            in_channels=int(self.n_embd),
            out_channels=int(self.n_embd),
            hidden_channels=128,
            num_layers=3,
            norm="layer_norm",
            act="relu",
    ...
    def forward(self, data):
        ...
        x, coords, mask, kwargs = prepare_input(x, edge_index, coords, batch, self.helper_funcs)
        ...
        if self.task == "pileup":
            ...
            out = out.view(-1,int(self.n_embd))
