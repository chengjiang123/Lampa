## from https://github.com/Graph-COM/HEPT/tree/main/src/datasets
...
class PileupTransform(BaseTransform):
    def __call__(self, data):
        ...
        data.coords = torch.cat([data.pos, data.x[:, 6:7]], dim=-1)
        return data
...
class Pileup(InMemoryDataset):
    ...
    def process(self):
        ...
        data_dir = '/test_snapshot.root'
        
        train_data = prepare_dataset(data_dir, entry_start=0, entry_stop=20000)
        val_test_data = prepare_dataset(data_dir, entry_start=20000, entry_stop=24000)
...
def prepare_dataset(datadir, entry_start=0, entry_stop=None):
    features = [
        "PF/PF.PT",
        "PF/PF.Eta",
        "PF/PF.Phi",
    ...
    # Open the ROOT file from the EOS path
    tree = uproot.open(f"{datadir}")["Delphes"]
    num_entries = tree.num_entries if entry_stop is None else entry_stop - entry_start

    # Load particles data for the specified entry range
    particles = tree.arrays(features, entry_start=entry_start, entry_stop=entry_stop)
    data_list = Parallel(n_jobs=2, prefer="threads")(
...
def process_one_event(event):
    ...
    puppi = ak.to_numpy(event["PF/PF.PuppiWeight"])
    ...
    puppi_weight = torch.from_numpy(puppi)
    ...
    is_neu = is_neu[perm]
    pos = pos[perm]
    puppi_weight = puppi_weight[perm]
    #edge_index = to_undirected(knn_graph(pos, k=60, loop=True))
    #edge_index = to_undirected(knn_graph(pos, k=32, loop=True))

    #data = Data(x=node_features, edge_index=edge_index, y=label, pos=pos, is_neu=is_neu)
    data = Data(x=node_features, edge_index=0, y=label, pos=pos, is_neu=is_neu, puppi_weight=puppi_weight)
    return data
