...
def train_one_batch(model, optimizer, criterion, data, lr_s):
    ...
    num_neu = (data.is_neu == 1).sum().item()  # If data.is_neu is a tensor
...
def run_one_epoch(model, optimizer, criterion, data_loader, phase, epoch, device, metrics, lr_s):
    ...
    for idx, data in enumerate(pbar):
        ...
        #batch_roc = update_metrics(metrics, data, batch_embeddings)
        metrics["loss"].update(batch_loss)

        desc = f"[Epoch {epoch}] {phase}, loss: {batch_loss:.4f}, auc: {batch_auc:.4f}"
        #desc = f"[Epoch {epoch}] {phase}, loss: {batch_loss:.4f}, roc: {batch_roc:.4f}"
...
def run_one_seed(config, tune=False):
    ...
    #model_kwargs = {k: v for k, v in config["model_kwargs"].items() if k != 'model_name'}
    ...
    if config.get("resume", False):
        ...
        model_path = dataset_dir / "logs" / (config["resume"] + "/A/best_model.pt")
    ...
    for epoch in range(config["num_epochs"]):
       ...
       # Clear CUDA memory at the end of each epoch
        torch.cuda.empty_cache()
...
def main():
    ...
    if args.model in ["mamba","hmamba","hydra","jamba"]:
        config_dir = Path(f"./configs/pileup/pileup_mamba_{args.model}.yaml")
    elif args.model in ["rwkv"]:
        config_dir = Path(f"./configs/pileup/pileup_rwkv_{args.model}.yaml")
    else:
