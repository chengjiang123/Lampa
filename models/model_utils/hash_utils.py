...
@torch.no_grad()
def lsh_mapping1(e2lsh, hidden_states):
    """
    Applies LSH (Locality Sensitive Hashing) to the hidden states for SSM, 
    capturing locality and efficient lookup using LSH.
    """
    # Apply LSH to hidden states
    hidden_hashed = e2lsh(hidden_states)

    # Calculate hash shift using the min and max values of the hashed hidden states
    max_hash_shift = torch.max(hidden_hashed, dim=-1, keepdim=True).values
    min_hash_shift = torch.min(hidden_hashed, dim=-1, keepdim=True).values
    hash_shift = max_hash_shift - min_hash_shift

    return hidden_hashed, hash_shift
